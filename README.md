...
The command discovers the remote primary branch, merges in an isolated temporary worktree, reruns configured gates, pushes without force, verifies the remote commit, and only then removes the completed linked worktree and local/remote work branches. Dirty, diverged, detached, conflicting, stale-evidence, or concurrently changed states fail closed without deleting work.

Use `$ARGUMENTS` with `--dry-run` to preview the decision and exact Git operations without changing state. A successful summary classifies the delivery as product, feature, bugfix, security, refactor, documentation, chore, or other.
...
Use it to require subagents, tune parallel capacity, define test/build/lint/typecheck commands, configure review/Constitution validation, select the remote/base branch, and control post-merge cleanup.
